...
# si ya existen helpers, conservá los tuyos
# from app.security.billing_guard import activate_user_pro  # si ya lo tenés
...
async def mp_webhook(request: Request, db: Session = Depends(get_db), x_signature: str | None = Header(default=None)):
    ...
    if secret:
        if not x_signature or not _hmac_valid(secret, raw, x_signature):
    ...
    if not mp_payment_id:
        # Algunos eventos envían solo topic y resource; opcional: resolver por query
        raise HTTPException(status_code=400, detail="Missing payment id")

    # Fuente de verdad: consultamos el pago en MP
    r = requests.get(f"https://api.mercadopago.com/v1/payments/{mp_payment_id}", headers=_mp_headers(), timeout=REQ_TIMEOUT)
    info = r.json()
    ...
    # Activación PRO (ajustá a tu helper si el nombre difiere)
    if status in ("approved", "authorized") and user_id:
        try:
            ...
            # Si no existe helper, el alta puede quedar a cargo de normalize_user_plan
            pass
