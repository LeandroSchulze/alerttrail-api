...
from app.models import MailAccount, PushSubscription # Importamos el modelo de suscripción
...
def mail_scanner(request: Request, user=Depends(get_current_user_cookie_optional), db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request=request, name="mail_scanner.html", context={"lang": lang, "t": t_func, "user": user, "linked": linked, "last_scan": {"ts": last_scan_raw.get("scanned_at") or "", "total": last_scan_raw.get("total", 0)}, "scan_items": last_scan_raw.get("items", [])})
...
def scan_get(request: Request, user=Depends(get_current_user_cookie_optional), db: Session = Depends(get_db), limit: int = Query(20)):
    ...
    for it in (res.items or []):
        ...
        items.append({
            ...
            "verdict": final_lvl, # Campo que intentamos usar
            "danger_level": final_lvl.lower(), # Algunos templates usan este
    ...
    if high_threat_found and uid:
        # --- DEBUG DE SUSCRIPCIONES ---
        subs = db.query(PushSubscription).filter(PushSubscription.user_id == uid).all()
        print(f"🔔 AMENAZA ALTA. Suscripciones encontradas en DB para usuario {uid}: {len(subs)}")
        
        if len(subs) > 0:
            print(f"🚀 Enviando notificación a {len(subs)} dispositivos...")
            trigger_push_notification(user_id=uid, title="🚨 ALERTA CRÍTICA", body="Se detectó un intento de hackeo.")
        else:
            print("⚠️ El push se disparó pero NO hay dispositivos suscritos en la base de datos.")
