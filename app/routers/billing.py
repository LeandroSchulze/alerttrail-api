...
def billing_page(request: Request, db: Session = Depends(get_db), all: int = Query(0)):
    ...
    # Mantenemos el contexto que espera tu template: 'payments'
...
def billing_payments(db: Session = Depends(get_db), request: Request = None, all: int = Query(0)):
    """Endpoint que tus links usan (url_for('billing_payments')). Devuelve JSON simple."""
    user = get_current_user_cookie(request, db)
...
def create_mp_checkout(request: Request, db: Session = Depends(get_db)):
    """Crea preferencia de MP y devuelve init_point (no modifica tu template)."""
    user = get_current_user_cookie(request, db)
    ...
    if not notification_url:
        # usa tu webhook si no seteaste MP_WEBHOOK_URL
        try:
            notification_url = str(request.url_for("payments_mp_webhook")).replace("http://", "https://")
        except Exception:
            raise HTTPException(status_code=500, detail="Configura MP_WEBHOOK_URL")
    ...
    r = requests.post(
        ...
        timeout=int(os.getenv("MP_REQ_TIMEOUT_SEC", "25")),
    )
    data = r.json()
