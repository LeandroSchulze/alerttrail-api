...
def _extract_level_and_reasons(item: Dict[str, Any]) -> Tuple[str, list]:
    ...
    Normaliza diferentes formatos de items:
      ...
      - Compat: item['danger_level']
      - Legacy: item['level'] + item['reasons']
      - Legacy UI: item['verdict'] ('ALTO'/'MEDIO'/'BAJO')
    """
    analysis = item.get("analysis") or {}
    if not isinstance(analysis, dict):
        analysis = {}

    level = (analysis.get("danger_level") or item.get("danger_level") or item.get("level") or "").strip().lower()
    if not level:
...
def alerts_pending(request: Request):
    ...
    for it in _items_newest_first(items):
        ...
        if last_delivered and mail_id == last_delivered:
            # ya entregado (y como viene newest-first, no hay más nuevos)
            return JSONResponse({"ok": True, "pending": False})
