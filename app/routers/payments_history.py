...
def payments_list(
    ...
    # Build base query (texto SQL para ser agnóstico al ORM del proyecto)
    base = "SELECT id, payment_id, user_id, email, status, currency, amount, external_reference, paid_at, created_at FROM payments"
    where = []
    params = {}

    # Permisos: si NO es admin, solo sus pagos (por user_id o email)
    ...
    if role != "admin":
        if getattr(u, "id", None):
            ...
            # sin id ni email… no mostramos nada
            where.append("1=0")
    else:
        if email:
    ...
    # Render rápido en HTML básico (sin template para no depender de archivos)
