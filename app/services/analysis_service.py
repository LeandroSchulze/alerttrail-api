...
def analyze_log(
    ...
    *,
    ...
    """
    Analiza logs de seguridad y devuelve un resumen de riesgo.
    Si el riesgo es MEDIUM o HIGH y hay user_id, dispara alerta push.
    """
    ...
    # 🔥 DISPARADOR REAL DE ALERTAS (CAMBIO CLAVE)
    if risk in ("medium", "high") and user_id:
        notify_all(
            user_id=user_id,
            to_email=user_email,
            subject="⚠️ Alerta de seguridad",
            body="Se detectó actividad sospechosa en los logs.",
            extra={
                "risk": risk,
                "stats": summary,
            },
