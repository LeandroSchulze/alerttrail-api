...
# Routers (tu __init__.py de routers los importa lazy)
...
# Sesiones (por si usás flash u otros)
app.add_middleware(SessionMiddleware, secret_key=SESSION_SECRET)


class LangHeaderMiddleware(BaseHTTPMiddleware):
    """Agrega Content-Language según cookie/query lang (usa get_lang)."""
...
def dashboard(request: Request):
    ...
    if not user:
        # Importante: tu login UI real está en /auth/login
...
# ✅ Alias /login -> /auth/login (evita el 404 que te aparece en Render)
@app.get("/login", include_in_schema=False)
def login_alias():
    return RedirectResponse(url="/auth/login", status_code=302)


# ✅ Alias /logout -> /auth/logout (opcional pero práctico)
