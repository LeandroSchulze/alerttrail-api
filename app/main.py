...
async def set_lang(
    ...
    """
    # Normalizamos el idioma pedido (ya validado por el pattern)
    selected_lang = (lang or "es").lower()
...
# ---- Fallback UI para /audit si no hay router dedicado ----
if not _route_exists_ui("/audit"):
    from typing import Any as _Any

    def _audit_get_user_email(user: _Any) -> str:
        if user is None:
            return ""
    ...
    def _audit_landing(request: Request, user=Depends(get_current_user_cookie)):
        user_email = _audit_get_user_email(user)
        html = f"""<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    .audit-muted{{ color:#64748b;font-size:14px; }}
    .audit-card-title{{ font-size:18px;font-weight:600;margin-bottom:4px; }}
    .audit-list{{ margin:8px 0 0 18px;padding:0;color:#64748b;font-size:14px; }}
    ...
    .audit-input,.audit-textarea,.audit-select{{
        ...
        padding:8px 10px;font:inherit;color:#0f172a;background:#fff;
    }}
    ...
    .audit-help{{ font-size:12px;color:#64748b;margin-top:4px; }}
...
<body>
  <div class="audit-page">
    ...
    @app.post("/audit/request", response_class=HTMLResponse)
    def _audit_request(
        request: Request,
        who: str = Form(...),
        ...
        try:
            ...
            target = os.getenv("AUDIT_REQUEST_EMAIL", "info.alerttrail@gmail.com")
            _send_email(target, subject, plain, html_body)
        except Exception as e:
            send_ok = False
            error_msg = str(e)
